...
<#
.SYNOPSIS
    Renders Build-SlackPayload against a synthetic fixture for visual review.

.DESCRIPTION
    Writes the JSON payload to tests/output/slack-payload.json. Paste the full
    payload into https://app.slack.com/block-kit-builder/ to preview rendering.
    Pass -WithReportUrl to simulate REPORT_ARTIFACT=true and an Azure DevOps run.
#>
[CmdletBinding()]
param(
...
. (Join-Path $src 'notifications-webhook-slack.ps1')
...
$outPath = Join-Path $PSScriptRoot '../output/slack-payload.json'
$json    = $payload | ConvertTo-Json -Depth 30
Set-Content -Path $outPath -Value $json -Encoding UTF8

Write-Host "Preview written:    $outPath"
Write-Host "Block count:        $($payload.blocks.Count) / 50"
Write-Host "Payload size:       $($json.Length) chars"
Write-Host "Report URL injected: $WithReportUrl"
Write-Host ''
Write-Host 'Paste the full JSON into https://app.slack.com/block-kit-builder/ to preview.'
